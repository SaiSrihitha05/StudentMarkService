            ...
            steps {
                script {
                    withEnv(["KUBECONFIG=C:\\Users\\HP\\.kube\\config"]) {
                        ...
                        bat '''
powershell -Command "(Get-Content k8s/namespace-template.yaml) `
    -replace '\\${APP_NAMESPACE}', '${APP_NAMESPACE}' `
    -replace '\\${APP_NAME}', '${APP_NAME}' |
    Set-Content k8s/namespace.yaml"

powershell -Command "(Get-Content k8s/deployment-template.yaml) `
    -replace '\\${APP_NAME}', '${APP_NAME}' `
    -replace '\\${APP_NAMESPACE}', '${APP_NAMESPACE}' `
...
powershell -Command "(Get-Content k8s/service-template.yaml) `
    -replace '\\${APP_NAME}', '${APP_NAME}' `
    -replace '\\${APP_NAMESPACE}', '${APP_NAMESPACE}' `
    -replace '\\${APP_PORT}', '${APP_PORT}' `
